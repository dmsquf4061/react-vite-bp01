...
function SectionLibraries() {
  const libraries = [
    {
      label: "GSAP",
      description: "스크롤 기반 고급 애니메이션 구현",
      thumbnail: "/libraries/gsap.webp",
    },
    {
      label: "Motion",
      description: "UI 전환 및 컴포넌트 인터랙션 구현",
      thumbnail: "/libraries/motion.webp",
    },
    {
      label: "Swiper",
      description: "슬라이드 UI 및 캐러셀 구현",
      thumbnail: "/libraries/swiper.webp",
    },
  ];
    ...
    <section id="libraries" className="w-full h-screen">
      <ul className="flex w-full h-full flex-col md:flex-row justify-between">
        ...
        <li className="flex w-full md:w-[calc(100vw-500px)] h-[calc(100vh-220px)] md:h-full items-center justify-center md:justify-start">
          {currentIndex === null ? (
            <h1 className="flex flex-col font-extrabold leading-none md:ml-15 text-5xl md:text-7xl md:text-left">
            ...
            <div
              role="tabpanel"
              className="flex w-full h-full flex-col gap-4 md:gap-8 p-4 sm:p-10 md:p-15 pt-18 sm:pt-22 md:pt-35"
            >
              <div className="w-full h-full rounded-lg overflow-hidden bg-[#B0B0B0]">
                <img
                  src={libraries[currentIndex].thumbnail}
                  alt={libraries[currentIndex].label}
